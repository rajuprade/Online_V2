          ...
          // for(;;)
           while(1)
           {
               ...
               { 
                    ...
                    a1 = (ANT_CMD *) q_retrieve((int)Q.retrieve);
                    pthread_mutex_unlock(&lock);
                    if((ANT_CMD *)a1 != (ANT_CMD *)NULL)
                   {  
                     if(!strcasecmp(a1->antenna_name,"C00"))
                     {
                        tstamp = tp(tstamp);
                       ...
                       if((bytes = writen(sfd,(unsigned char *)a1,sizeof(ANT_CMD)))!=sizeof(ANT_CMD))
                        ...
                        { //fprintf(stderr," ##### ANTENNNA Name is %s #####\n",a1->antenna_name); 
                           //fprintf(stderr,"SIze of ANT_CMD is %s\n",sizeof(a1));
                           c1 = &a1->CMD;
                          // printcmd(c1);
                        }
...
void *C01_handler(void *socket_desc)
{
     int sfd=0,n;
     ssize_t bytes=-1;
    ...
    // strcpy(a1->antenna_name,Antenna_data->aname); // copy the first antenna name from file/user input 
     ...
     //fprintf(stderr," SOCK FD is %d\n",sfd);
      Q1.store = 0; Q1.retrieve = 0;
           for(;;)
           {
               while((int)(Q1.store)>(int)(Q1.retrieve))
               { 
                  
                    pthread_mutex_lock(&lock);
                    a1 = (ANT_CMD *)q1_retrieve((int)Q1.retrieve);
                    pthread_mutex_unlock(&lock);
                    if((ANT_CMD *)a1 != (ANT_CMD *)NULL)
                   {  
                     if(!strcasecmp(a1->antenna_name,"C01"))
                     {
                        tstamp = tp(tstamp);
                        strcpy(a1->CMD.timestamp,tstamp);
                       if((bytes = writen(sfd,(unsigned char *)a1,sizeof(ANT_CMD)))!=sizeof(ANT_CMD))
