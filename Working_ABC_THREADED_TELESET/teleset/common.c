...
void *connection_handler(void *socket_desc)
{
     ...
     fprintf(stderr," SOCK FD is %d\n",sfd);
           for(;;)
           {
             ...
             //bytes = writen(sfd,(void *)a1,sizeof(ANT_CMD));
               bytes = write(sfd,a1,sizeof(ANT_CMD));
               if(bytes < sizeof(ANT_CMD))
                 fprintf(stderr,"\nANT CMD Command writing Failed !!!!\n"); 
              sleep(1);
             fprintf(stderr,"\nNow Reading on Socket\n"); 
              ...
              {   bytes = -1;         
                //  bytes=readline(sfd,(void *)ack,sizeof(ACK));
                    bytes = read(sfd,ack,sizeof(ACK));
                  if (bytes < sizeof(ACK))
                  fprintf(stderr,"ACK writing Failed !!!!!!\n");
                  else
            ...
            // sleep(1);
              c1 = &a1->CMD[0];
              bytes = -1;
            //   bytes =readline(sfd,(void *)b1,sizeof(ANT_RESP));
                 bytes = read(sfd,b1,sizeof(ANT_RESP));
               if (bytes < sizeof(ANT_RESP))
